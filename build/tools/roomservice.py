...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from Creative-OS Github (http://github.com/CreativeOS)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/search/repositories?q=%s+user:Creative-OS+in:name+fork:true" % device)
    add_auth(githubreq)
    try:
...
def is_in_manifest(projectpath):
    ...
    # ... and don't forget the creative snippet
    try:
        lm = ElementTree.parse(".repo/manifests/snippets/creative.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('Creative-OS/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: Creative-OS/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "Creative-OS/%s" % repo_name,
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/creative.dependencies'
    syncable_repos = []
    verify_repos = []
...
def get_default_or_fallback_revision(repo_name):
    ...
    githubreq = urllib.request.Request("https://api.github.com/repos/Creative-OS/" + repo_name + "/branches")
    add_auth(githubreq)
    result = json.loads(urllib.request.urlopen(githubreq).read().decode())
...
print("Repository for %s not found in the Creative-OS Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
