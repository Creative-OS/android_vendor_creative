function __print_creative_functions_help() {
cat <<EOF
Additional CreativeOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- creativegerrit:   A Git wrapper that fetches/pushes patch from/to Creative-OS Gerrit Review.
- creativerebase:   Rebase a Gerrit change and push it again.
- creativeremote:   Add git remote for Creative-OS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for Creative-OS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Creative model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch creative_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/creative-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.creative.device | grep -q "$CREATIVE_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $CREATIVE_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function creativeremote()
{
    ...
    git remote rm creative 2> /dev/null
    ...
    then
        ...
        local PFX="Creative-OS/"
    else
        local PROJECT=$REMOTE
    fi

    local CREATIVE_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$CREATIVE_USER" ]
    then
        git remote add creative ssh://review.lineageos.org:29418/$PFX$PROJECT
    else
        git remote add creative ssh://$CREATIVE_USER@review.lineageos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'creative' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/Creative-OS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.creative.device | grep -q "$CREATIVE_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        creativeremote
        git push creative HEAD:refs/heads/'$1'
    fi
    '
}

function creativegerrit() {
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "creativegerrit" ]; then
...
function creativerebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Creative-OS Gerrit Rebase Usage: "
        echo "      creativerebase <path to project> <patch IDs on Gerrit>"
...
function dopush()
{
    ...
    if (adb shell getprop ro.creative.device | grep -q "$CREATIVE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $CREATIVE_FIXUP_COMMON_OUT ]; then
